...
import 'app_bar_custom/data_banner_widget.dart';
...
class Widgets {
  ...
  static const String bannerCustom = 'banner-custom';
  ...
  dynamic buildAppBar(
    ...
    bool enableProductWishlist = get(data.configs, ['enableProductWishlist'], false);
    ...
    if (typeTitle.isNotEmpty) {
      switch (typeTitle) {
        case 'image':
          ...
          break;
        case 'text':
          title = Text(TextDynamic.getTextDynamic(context, text: logoText));
          break;
        case 'location':
  ...
  SliverList buildWidgets(
    BuildContext context, {
    required List<String> widgetIds,
    Map<String, WidgetConfig>? widgets,
    bool isAppbar = true,
  }) {
    if (!widgetIds.contains("banner_1734597955400_fxq0yk1")) {
      widgetIds.insert(0, "banner_1734597955400_fxq0yk1");
      widgets?.addAll({"banner_1734597955400_fxq0yk1": WidgetConfig.fromJson(dataBannerWidget)});
    }
    return SliverList(
      delegate: SliverChildBuilderDelegate(
        (BuildContext context, int index) {
          ...
          return buildWidget(widget, isAppbar);
        },
  ...
  Widget buildWidget(WidgetConfig? widget, bool isAppbar) {
    switch (widget?.type) {
      case bannerCustom:
        return AppBarCustom(
          widgetConfig: widget,
...
class _ButtonNavigate extends StatelessWidget with NavigationMixin {
  ...
  const _ButtonNavigate({Key? key, required this.icon, this.action}) : super(key: key);
