...
import 'package:cirilla/store/state_base/base_state.dart';
...
class _DiscountPointApplyState extends State<DiscountPointApply>
  ...
  @override
  void initState() {
    _maxPoint = get(widget.cartStore.cartData?.extensions,
            ['points-and-rewards', 'cart_max_points'], '0')
        .toString();
  ...
  Future<void> _applyPoint(BuildContext context) async {
    ...
    if (!loading && controller.text.trim().isNotEmpty) {
      ...
      try {
        ...
        await widget.cartStore.applyPointDiscount(
            value: value, cartKey: widget.cartStore.cartKey ?? '');
        setState(() {
          _loading = false;
        });
        if (context.mounted)
  ...
  _checkShowAddPoint(CartStore cartStore) {
    Map<String, dynamic>? pointCoupon = cartStore.cartData?.coupons
        ?.firstWhereOrNull(
            (element) => element['code'].contains('wc_points_redemption'));
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(
      builder: (context) {
        ...
        String? message = get(widget.cartStore.cartData?.extensions,
            ['points-and-rewards', 'redeem_points_message'], null);
